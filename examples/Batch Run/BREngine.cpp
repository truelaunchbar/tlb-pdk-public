...
CBREngine::CBREngine(HINSTANCE hInst) : CXUIEngine(hInst)
{
}

CBREngine::~CBREngine(void)
{
}

void CBREngine::DrawImage( HDC hdc, int x, int y, int width, int height, LPWSTR imgUrl )
{
	CTxDIB img;
	...
	if(img.isValid())
	{
		if(!lstrcmpi(L"delhkicon.png", imgUrl))
		{
			...
			if(width < 24)
			{
				if(width < 20)
				{
					img.load(FindResource(m_hInst, L"delhkicon16.png", TEXT("XUILIB")), m_hInst);
					width = height = 16;
				} else
				{
					img.load(FindResource(m_hInst, L"delhkicon20.png", TEXT("XUILIB")), m_hInst);
					width = height = 20;
				}
			} else
			{
...
void CBREngine::DrawFrame( HDC hdc, int x, int y, int width, int height, LPWSTR imgUrl, int frames, int frame, int framesOrient )
{
	CTxDIB img;
	img.load(FindResource(m_hInst, imgUrl, TEXT("XUILIB")), m_hInst);
	if(img.isValid())
